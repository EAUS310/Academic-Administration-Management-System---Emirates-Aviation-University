// gen-module-tracker.js — GEN Modules Tracker (GitHub Pages static version)
...
(function init() {
  ...
  const { sections, modules } = computeModuleSections(d.students, (item, type) => {
    if (type === 'module') return item.code.toUpperCase().startsWith('GEN');
    return true;
  });
...
function downloadPDF() {
  ...
  doc.text('All GEN Modules Tracker', 14, 14);
  ...
  doc.save('GEN_Modules_Tracker.pdf');
}
