// other-report.js — Other Programs Attendance Report (GitHub Pages static version)
...
try {
  ...
  if (d && d.generated) {
    document.getElementById('otherStatus').textContent =
      `Last updated: ${new Date(d.generated).toLocaleString()}`;
    loadOtherData();
  }
} catch (e) {
  console.error('[other-report] loadStatus failed:', e);
}
...
async function doUpload() {
  ...
  try {
    ...
    document.getElementById('otherStatus').textContent = 'Last updated: just now';
    loadOtherData();
  } catch (err) {
...
function loadOtherData() {
  ...
  for (const m of data.modules) {
    ...
    for (const s of m.students) {
      ...
      if (prog.includes('aircraft maintenance engineering')) continue;
      if (s.warnPct < 25) continue;
      tableData.push({
        studentId:   s.id,
        studentName: s.name,
        program:     s.program,
  ...
  if (!tableData.length) {
    document.getElementById('emptyState').querySelector('p').textContent =
      'No non-AME students with attendance warnings found.';
    return;
  }

  document.getElementById('emptyState').style.display    = 'none';
  document.getElementById('otherTableWrap').style.display = 'block';
  document.getElementById('excelBtn').style.display       = 'flex';
  document.getElementById('filterBar').style.display      = '';

  const studentIds = [...new Set(tableData.map(r => r.studentId))].sort();
  msOtherStudents = new MultiSelect('ms-other-students', 'Select student IDs...');
  msOtherStudents.setOptions(studentIds);

  renderOtherTable(tableData);
}

function resetOtherFilter() {
  msOtherStudents && msOtherStudents.clear();
  renderOtherTable(tableData);
}

function renderOtherTable(rows) {
  document.getElementById('otherCount').textContent = `${rows.length} record${rows.length !== 1 ? 's' : ''}`;
  document.getElementById('otherBody').innerHTML = rows.map(r => `
    <tr>
...
function applyFilters() {
  ...
  const sel = msOtherStudents ? msOtherStudents.getSelected() : [];
  renderOtherTable(sel.length ? tableData.filter(r => sel.includes(r.studentId)) : tableData);
}

function downloadExcel() {
  if (!tableData.length) return;
  const wsData = [
    ['Student ID', 'Student Name', 'Program', 'Module Code', 'Module Name', 'Warning %'],
    ...tableData.map(r => [r.studentId, r.studentName, r.program, r.moduleCode, r.moduleName, r.warnPct])
  ];
  const ws = XLSX.utils.aoa_to_sheet(wsData);
  ws['!cols'] = wsData[0].map((_, ci) => ({ wch: Math.max(...wsData.map(row => String(row[ci]??'').length)) + 2 }));
  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, ws, 'Other Programs Warnings');
  const buf  = XLSX.write(wb, { type: 'array', bookType: 'xlsx' });
  const blob = new Blob([buf], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
  const url  = URL.createObjectURL(blob);
  const a    = document.createElement('a');
  a.href = url; a.download = 'Other_Programs_Attendance_Warning.xlsx'; a.click();
