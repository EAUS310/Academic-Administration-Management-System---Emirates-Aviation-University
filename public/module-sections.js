// module-sections.js — All Modules Tracker (GitHub Pages static version)
...
(function init() {
  ...
  const { sections, modules } = computeModuleSections(d.students, (item, type) => {
    ...
    if (type === 'module') {
      ...
      if (SKIP_PREFIXES.some(p => cu.startsWith(p))) return false;
      if (cu.startsWith('ENG') && !ENG_WHITELIST.has(cu)) return false;
      return true;
    }
    return true;
  });
...
function applyFilter() {
  ...
  const filtered = q ? allModules.filter(m => m.code.toUpperCase().includes(q)) : allModules;
...
function downloadPDF() {
  ...
  doc.text('All Modules Tracker', 14, 14);
  doc.setFontSize(8);
  ...
  const body = modules.map((m, i) => [
    i + 1, m.code, m.name,
    ...allSections.map(s => m.sections[s] || '—'),
    m.total
  ]);
  ...
  doc.save('All_Modules_Tracker.pdf');
}
